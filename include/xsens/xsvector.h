/*	WARNING: COPYRIGHT (C) 2016 XSENS TECHNOLOGIES OR SUBSIDIARIES
   WORLDWIDE. ALL RIGHTS RESERVED. THIS FILE AND THE SOURCE CODE IT CONTAINS
   (AND/OR THE BINARY CODE FILES FOUND IN THE SAME FOLDER THAT CONTAINS THIS
   FILE) AND ALL RELATED SOFTWARE (COLLECTIVELY, "CODE") ARE SUBJECT TO A
   RESTRICTED LICENSE AGREEMENT ("AGREEMENT") BETWEEN XSENS AS LICENSOR AND THE
   AUTHORIZED LICENSEE UNDER THE AGREEMENT. THE CODE MUST BE USED SOLELY WITH
   XSENS PRODUCTS INCORPORATED INTO LICENSEE PRODUCTS IN ACCORDANCE WITH THE
   ...
   NOT AN AUTHORIZED USER OF THE CODE IN ACCORDANCE WITH THE AGREEMENT, YOU MUST
   STOP USING OR VIEWING THE CODE NOW, REMOVE ANY COPIES OF THE CODE FROM YOUR
   COMPUTER AND NOTIFY XSENS IMMEDIATELY BY EMAIL TO INFO@XSENS.COM. ANY COPIES
   OR DERIVATIVES OF THE CODE (IN WHOLE OR IN PART) IN SOURCE CODE FORM THAT ARE
   PERMITTED BY THE AGREEMENT MUST RETAIN THE ABOVE COPYRIGHT NOTICE AND THIS
   PARAGRAPH IN ITS ENTIRETY, AS REQUIRED BY THE AGREEMENT.
*/
...
XSTYPES_DLL_API void XsVector_ref(XsVector *thisPtr, XsSize sz, XsReal *buffer,
                                  XsDataFlags flags);
XSTYPES_DLL_API void XsVector_construct(XsVector *thisPtr, XsSize sz,
                                        const XsReal *src);
XSTYPES_DLL_API void XsVector_assign(XsVector *thisPtr, XsSize sz,
                                     const XsReal *src);
...
XSTYPES_DLL_API void XsVector_multiplyScalar(const XsVector *thisPtr,
                                             XsReal scalar, XsVector *dest);
XSTYPES_DLL_API void
XsVector_angularVelocityFromQuaternion(XsVector *thisPtr, XsReal deltaT,
...
XSTYPES_DLL_API int XsVector_compare(const XsVector *thisPtr,
                                     const XsVector *thatPtr, XsReal epsilon);
...
public:
  //! \brief Initialize a vector, empty or using the data in the supplied \a sz
  //! and \a src
  ...
  inline explicit XsVector(XsReal *ref, XsSize sz,
                           XsDataFlags flags_ = XSDF_None)
      : m_data(ref), m_size(sz), m_flags(flags_) {}

  //! \brief Initialize a vector that references the supplied data
  inline explicit XsVector(const XsVector &other, XsReal *ref, XsSize sz,
                           XsDataFlags flags_ = XSDF_None)
      : m_data(ref), m_size(sz), m_flags(flags_) {
  ...
  */
  ...
  inline void normalize() { XsVector_normalize(this); }
  ...
  //! \copydoc XsVector_angularVelocityFromQuaternion
  inline XsVector &angularVelocityFromQuaternion(const XsQuaternion &quat,
                                                 XsReal deltaT) {
    XsVector_angularVelocityFromQuaternion(this, deltaT, &quat);
    return *this;
  }
  ...
  XsVector operator/(const double &n) const {
    XsVector tmp(m_size);
    for (XsSize i = 0; i < m_size; ++i)
      tmp[i] = m_data[i] / n;
    return tmp;
  ...
  //! \brief Return true when the values in this vector are exactly (to the last
  //! bit) equal to \a other
  bool operator==(const XsVector &other) const {
    return 0 != XsVector_equal(this, &other);
  }

  /*! \brief Return true when the values in this vector are equal within \a
     epsilon \param other the vector to compare with \param epsilon the maximum
     difference between individual values \returns true if the vectors are equal
     within \a epsilon
   */
  ...
  /*! \brief Swap the contents of \a b with this
     ...
     moved around. For unmanaged data an elementwise swap is done, but only if
     the vectors are the same size. \param b Object whose contents will be
     swapped with this
  */
